      ...
      >
          ...
          {SOCIALS.map((s, i) => (
            <li key={s.key} role="none">
              ...
              >
                ...
                {/* Reposo: círculo marfil, icono tinta. Hover: color oficial de
                    la red + icono blanco (glitch cian/rosa en TikTok). */}
                <span className="relative flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-ivory text-ink shadow-[0_6px_28px_-8px_rgba(18,16,14,0.5)] ring-1 ring-ink/8 transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover/item:scale-105 group-active/item:scale-95">
                  <span
                    className="absolute inset-0 opacity-0 transition-opacity duration-500 group-hover/item:opacity-100"
                    style={{ background: BRAND[s.key].hoverBg }}
                  />
                  <SocialIcon
                    name={s.key}
                    size={17}
                    className={`relative transition-[color,filter] duration-500 group-hover/item:text-white ${
                      s.key === 'tiktok'
                        ? 'group-hover/item:[filter:drop-shadow(1.4px_0_0_#FE2C55)_drop-shadow(-1.4px_0_0_#25F4EE)]'
                        : ''
                    }`}
                  />
                </span>
        ...
        <div className="flex items-center gap-2.5">
          ...
          >
            ...
            {/* Verde oficial de WhatsApp al pasar el cursor. */}
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-ivory/10 text-ivory transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover/wa:scale-105 group-hover/wa:bg-[#25D366] group-hover/wa:text-white">
