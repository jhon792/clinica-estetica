  ...
  return (
    <section id="testimonios" className="py-24 bg-[#f9f6f0]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* Review Cards — desktop: todos / móvil: 2 primeros + Ver más */}
        <>
          {/* Desktop */}
          ...
          <div className="grid md:hidden grid-cols-1 gap-5">
            {(showAll ? reviews : reviews.slice(0, 2)).map(r => (
              <ReviewCard key={r.name} r={r} />
            ))}
            {!showAll && (
              <button
                onClick={() => setShowAll(true)}
                className="flex items-center justify-center gap-2 border-2 border-[#b8973e] text-[#b8973e] hover:bg-[#b8973e] hover:text-white text-[11px] font-bold tracking-[0.12em] uppercase px-6 py-4 transition-all duration-300"
              >
                Ver más opiniones
